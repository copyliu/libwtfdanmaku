...
namespace WTFDanmaku
{
    ...
    {
        mDanmakus = std::move(std::make_unique<std::vector<DanmakuRef>>());
    }
    ...
    bool BilibiliParser::ParseXml(const char* data)
    {
        ...
        for (auto node = root->first_node("d"); node != nullptr; node = node->next_sibling())
        {
            ...
            if (attributes.size() >= 8)
            {
                ...
                auto type = static_cast<DanmakuType>(std::stoi(attributes[1]));
                ...
                DanmakuRef danmaku = DanmakuFactory::CreateDanmaku(type, time, comment, textSize, textColor, timestamp,
                                                                   danmakuId);
                if (danmaku != nullptr)
                {
