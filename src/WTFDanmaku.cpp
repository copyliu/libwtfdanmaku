...
{
    auto instance = new WTF_Instance;
...
{
    auto controller = reinterpret_cast<Controller*>(instance->controller);
...
WTF_C_API int __stdcall WTF_InitializeOffscreen(WTF_Instance* instance, uint32_t initialWidth, uint32_t initialHeight)
{
    ...
    return controller->Initialize(nullptr, initialWidth, initialHeight);
}
...
WTF_C_API void __stdcall WTF_AddDanmaku(WTF_Instance* instance, int type, int64_t time, const wchar_t* comment,
                                        int fontSize, int fontColor, int64_t timestamp, int danmakuId)
{
    auto controller = reinterpret_cast<Controller*>(instance->controller);

    DanmakuRef danmaku = DanmakuFactory::CreateDanmaku(static_cast<DanmakuType>(type), time, std::wstring(comment),
                                                       fontSize, fontColor, timestamp, danmakuId);
    controller->GetManager()->AddDanmaku(danmaku);
}

WTF_C_API void __stdcall WTF_AddLiveDanmaku(WTF_Instance* instance, int type, int64_t time, const wchar_t* comment,
                                            int fontSize, int fontColor, int64_t timestamp, int danmakuId)
{
    auto controller = reinterpret_cast<Controller*>(instance->controller);
...
{
    auto wtfwindow = new WTFWindow(static_cast<HINSTANCE>(hInstance), nCmdShow);
    return reinterpret_cast<WTF_Window*>(wtfwindow);
}

WTF_C_API void __stdcall WTFWindow_Release(WTF_Window* window)
{
    auto wtfwindow = reinterpret_cast<WTFWindow*>(window);

    delete wtfwindow;
}

WTF_C_API void __stdcall WTFWindow_Initialize(WTF_Window* window, uint32_t dwExStyle, int width, int height,
                                              const wchar_t* title)
{
    auto wtfwindow = reinterpret_cast<WTFWindow*>(window);
...
WTF_C_API LRESULT __stdcall WTFWindow_DefaultWindowProc(WTF_Window* window, void* hwnd, uint32_t message, WPARAM wParam,
                                                        LPARAM lParam)
{
    auto wtfwindow = reinterpret_cast<WTFWindow*>(window);
