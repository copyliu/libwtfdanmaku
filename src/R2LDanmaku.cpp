...
namespace WTFDanmaku
{
    ...
    {
    public:
        ~R2LRetainer() override = default;

        void Add(DanmakuRef danmaku, Displayer* displayer, DanmakuConfig* config, time_t currentMillis) override
        {
            ...
            for (auto iter = mDanmakus->begin(); iter != mDanmakus->end(); ++iter)
            {
                ...
                {
                    time_t meetDuration = (item->GetSpeed() * (currentMillis - item->GetStartTime()) - item->GetWidth())
                        / speedDiff;
                ...
                {
                    danmaku->Layout(displayer, config, static_cast<float>(displayer->GetWidth()),
                                    -danmaku->GetHeight() - 1);
                    return;
                }
            }
